...
\begin{figure}[H]
	\centering
		\begin{tikzpicture}
			...
			% Styles
			\tikzset{
				block1/.style={draw, align=left, thick, rectangle, text width=1.2cm, minimum height=1.75cm},
			...
			% Layer 2 Nodes
			\node[block2, above right=1.65cm and 0.75cm of knl]   (sgn) {Sinal};
			\node[block2, above right=-0.325cm and 0.75cm of knl] (lin) {Linear};
			\node[block2, below right=-0.325cm and 0.75cm of knl] (sgm) {Sigmóide};
			\node[block2, below right=1.65cm and 0.75cm of knl]   (tan) {Tanh};

			% Layer 3 Nodes
			\node[block3, right=0.75cm of sgn] (a1) {\textit{Anti-Hebbian}};
			\node[block3, above=0.1cm  of a1]  (h1) {\textit{Hebbian}};
			\node[block3, below=0.1cm  of a1]  (r1) {\textit{Randon-Walk}};

			\node[block3, right=0.75cm of lin] (a2) {\textit{Anti-Hebbian}};
			\node[block3, above=0.1cm  of a2]  (h2) {\textit{Hebbian}};
			\node[block3, below=0.1cm  of a2]  (r2) {\textit{Randon-Walk}};

			\node[block3, right=0.75cm of sgm] (a3) {\textit{Anti-Hebbian}};
			\node[block3, above=0.1cm  of a3]  (h3) {\textit{Hebbian}};
			\node[block3, below=0.1cm  of a3]  (r3) {\textit{Randon-Walk}};

			\node[block3, right=0.75cm of tan] (a4) {\textit{Anti-Hebbian}};
