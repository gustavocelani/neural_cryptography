    ...
    \centering
        \begin{tikzpicture}
            ...
            \tikzset{
                ...
                dblock/.style={draw, align=center, thick, rectangle, dashed, text width=3.6cm, minimum height=0.6cm}
            }
            ...
            % Label Nodes
            \node[thick] at (0, -1) {$\tau^A$};
            \node[thick] at (0, -2) {$\tau^B$};

            % Arrows
            \draw[-stealth, thick] ([yshift= 5pt] tpmA.east)  -- ([yshift= 5pt] tpmB.west);
            \draw[-stealth, thick] ([yshift=-5pt] tpmB.west)  -- ([yshift=-5pt] tpmA.east);
            \path[draw, -stealth, thick, dashed] (input.west) -- ++(-2cm,0) -- ++(0,-1.48cm) -- (tpmA.west);
            \path[draw, -stealth, thick, dashed] (input.east) -- ++(+2cm,0) -- ++(0,-1.48cm) -- (tpmB.east);
